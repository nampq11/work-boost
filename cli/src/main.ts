...
import { startApiMode } from '@work-boost/api';
import { initBrain } from '@work-boost/brain';
import { runMigrationIfNeeded } from '@work-boost/data-provider';
import { Database } from '@work-boost/data-provider';
import { initializeServices, validateRequiredSecrets } from '@work-boost/services';
...
program
  ...
  .action(async () => {
    ...
    try {
      const validation = validateRequiredSecrets({ strict: false });
      if (!validation.valid) {
        logger.warn('Skipping migration (missing secrets): ' + validation.missing.join(', '));
        return;
      }

      const { db } = await initializeServices({ strict: false });
...
program
  ...
  .action(async () => {
    ...
    const secretValidation = validateRequiredSecrets({ strict: false });
...
program
  ...
  .action(async () => {
    ...
    try {
      const { db, agent, slack, telegram } = await initializeServices({ strict: true });
        ...
        switch (command) {
          ...
          case '/execute': {
            ...
            await brain.executePlan(planId, (progress) => {
              ...
              console.log(
                `  ${status} Step ${progress.step}/${progress.total}: ${progress.description}`,
              );
            });
          ...
          case '/remember': {
            ...
            const result = await brain.storeMemory(sessionId, 'fact' as any, content, {
              source: 'cli',
            });
            if (result.success) {
              console.log(`\n✓ Remembered: ${content}\n`);
