...
import type { DebtReminderSettings } from '@work-boost/data-schemas/debt.ts';
  ...
  if (enabled) {
    const frequencyText =
      currentFrequency === 'weekly'
        ? `Weekly (every ${settings.weeklyDay || 1}${getOrdinal(settings.weeklyDay || 1)} day of the week)`
